...
// v34: z-sign selected per-frame based on which produces more w > 0.
...
static int   g_lock   = -1;       // locked combo (z-sign selected per-frame)
...
static const char* ComboName(int ci)
{
    ...
    static char buf[32];
    // bit layout (ComboBuild): slot=bit2, view-maj=bit3, proj-orient=bit1, z=bit0
    snprintf(buf, sizeof(buf), "v%s%s/p%s/z%s",
        slotN[(ci >> 2) & 1], majN[(ci >> 3) & 1],
        majN[(ci >> 1) & 1], (ci & 1) ? "-" : "+");
    return buf;
}
...
// Count how many targets have w > 0 for a given VP (quick z-sign check)
// Use same threshold as WorldToScreen (0.001) for consistency
static int CountPositiveW(const float VP[16])
{
    int cnt = 0;
    for (int i = 0; i < g_nT; i++) {
        if (g_t[i].x == 0.f && g_t[i].y == 0.f && g_t[i].z == 0.f) continue;
        float w = g_t[i].x*VP[3] + g_t[i].y*VP[7] + g_t[i].z*VP[11] + VP[15];
        if (w > 0.001f) ++cnt;  // same threshold as W2S
    }
    return cnt;
}
...
bool Init(void* device, void* immediateContext)
{
    ...
    oilog::Line("VPINIT: v35 frozen z-sign (lock-score authoritative)");
...
void OnFrame()
{
    ...
    if (!g_haveRaw) {
        ...
        // ---- locked: publish VP + eye, z-sign FROZEN at the locked combo.
        // v34 regression fix: the per-frame w-count vote flipped conventions
        // whenever the player turned toward clusters of behind-camera targets
        // (majority of 64 proxies are behind you in most spots) — boxes
        // vanished in front and behind-stuff drew mirrored/tiny. The lock
        // score (0.45+, measured on-screen) is authoritative; only the
        // 5 s score-based recheck below may change the convention.
        int baseCombo = g_lock & ~1;  // strip z-bit
        float VP0[16], eye[3];

        // Build only the LOCKED z-sign variant
        int usedZ = g_lock & 1;
        bool ok0 = ComboBuild(baseCombo | usedZ, VP0, eye);

        if (ok0) {
            float* VP = VP0;
            
            // Periodic recheck of base combo (slot/interpretation)
            DWORD now = GetTickCount();
            if (g_recheck == 0 || now - g_recheck > 5000) {
                g_recheck = now;
                float s = ComboScore(baseCombo | usedZ);
                if (s >= 0.f && s < 0.15f) {
                    // Try other base combos
                    float bb = -1.f; int bn = -1;
                    for (int ci = 0; ci < 16; ci += 2) {
                        float t0 = ComboScore(ci | 0);
                        float t1 = ComboScore(ci | 1);
                        float t = (t0 > t1) ? t0 : t1;
                        if (t > bb) { bb = t; bn = ci; }
                    }
                    if (bn >= 0 && (bn & ~1) != baseCombo && bb > 0.6f) {
                        g_lock = bn;
                        char buf[96];
...
bool WorldToScreen(float wx, float wy, float wz, float* sx, float* sy)
{
    ...
    // Clip check - widened to ±3.0 to allow partial off-screen entities
    if (nx < -3.0f || nx > 3.0f || ny < -3.0f || ny > 3.0f) { ++s_w2sClip; return false; }

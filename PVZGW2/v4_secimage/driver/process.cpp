...
static PVOID ResolveForward(const char* forward, ULONG_PTR /*modBase*/)
{
    ...
    if (!targetMod) {
        // api-set virtual DLLs (api-ms-win-*, ext-ms-*) are not in the PEB
        // module list. Their implementations live in host DLLs:
        //   api-ms-win-crt-*  -> ucrtbase.dll
        //   api-ms-win-core-* -> kernelbase.dll
        //   ext-ms-* / other api-ms-* -> try kernelbase.dll
        if (_strnicmp(dllName, "api-ms-win-crt-", 15) == 0)
            targetMod = GetUserModuleBase(L"ucrtbase.dll");
        else if (_strnicmp(dllName, "api-ms-win-core-", 16) == 0 ||
                 _strnicmp(dllName, "ext-ms-", 7) == 0 ||
                 _strnicmp(dllName, "api-ms-", 7) == 0)
            targetMod = GetUserModuleBase(L"kernelbase.dll");
    }

// Menu state from overlay.cpp
extern bool Overlay_IsMenuOpen();
...
bool c_widget::begin_popup(std::string_view name, float size_w, const ImVec2& position)
{
    ...
    char popup_name[64]; snprintf(popup_name, sizeof(popup_name), "%u - popup", id);
    ...
    if (!state->window_opened)
        popup_flags |= ImGuiWindowFlags_NoMouseInputs;
...
bool c_widget::set_tooltip(std::string_view tooltip_id, std::string_view tooltip_text)
{
    ...
    char popup_name[64]; snprintf(popup_name, sizeof(popup_name), "%u - popup", id);
    gui->begin(popup_name, nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollWithMouse);
    {

        state->hovered = IsMouseHoveringRect(GetWindowPos(), GetWindowPos() + GetWindowSize());
        content_size = GetContentRegionAvail();

        text_colored(set->c_font.icon[0], gui->get_clr(clr->c_other_clr.white_clr), "\xef\x81\x9a");
...
void c_widget::section(std::string_view label, const char* icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    ImDrawList* dl = window->DrawList;
    const ImVec2 pos = window->DC.CursorPos;
    const float w = GetContentRegionAvail().x;
    const float h = SCALE(26);
    const float accent_w = SCALE(3);
    const float icon_w = (icon && icon[0]) ? SCALE(20) : 0;
    const float pad_l = SCALE(8);
    ...
    // Accent bar (left edge, accent color)
    draw->add_rect_filled(dl, pos, { pos.x + accent_w, pos.y + h }, gui->get_clr(clr->c_other_clr.accent_clr), SCALE(1.f));

    // Icon (if provided)
    float text_x = pos.x + accent_w + pad_l;
    if (icon && icon[0]) {
        ImVec2 icon_min = { text_x, pos.y };
        ImVec2 icon_max = { text_x + icon_w, pos.y + h };
        draw->render_text(dl, set->c_font.icon[0], icon_min, icon_max, gui->get_clr(clr->c_other_clr.accent_clr), icon, 0, 0, { 0.5f, 0.5f });
        text_x += icon_w;
    }

    // Label
    ImVec2 txt_min = { text_x, pos.y };
    ImVec2 txt_max = { pos.x + w, pos.y + h };
    draw->render_text(dl, set->c_font.inter_medium[0], txt_min, txt_max, gui->get_clr(clr->c_text.text_hov), label.data(), 0, 0, { 0.0f, 0.5f });

    // Subtle bottom separator
    draw->add_line(dl, { pos.x, pos.y + h - SCALE(1) }, { pos.x + w, pos.y + h - SCALE(1) }, gui->get_clr(clr->c_element.separator), 1.f);
}
...
void c_gui::water_mark(std::string name, std::vector<std::string> function, watermark_position type, bool* visible)
{
    ...
    struct spectator_state { float alpha; };
    ...
    if (state->alpha >= 0.01f) {
        bool menu_open = Overlay_IsMenuOpen();
        
        // Use saved position
        ImVec2 pos(var->c_watermark.pos_x, var->c_watermark.pos_y);
        ...
        // Set window flags - draggable when menu open, locked when closed
        ...
        gui->begin("watermark", nullptr, flags);
        {
            // If menu open and being dragged, save new position
            if (menu_open) {
                ImVec2 win_pos = ImGui::GetWindowPos();
                if (win_pos.x != pos.x || win_pos.y != pos.y) {
                    var->c_watermark.pos_x = win_pos.x;
                    var->c_watermark.pos_y = win_pos.y;
                }
            }
            
            ImDrawList* draw_list = ImGui::GetWindowDrawList();
            
            // Determine separator line color - accent/RGB or custom
            ImU32 line_col;
            if (var->c_watermark.use_accent_color) {
                line_col = gui->get_clr(clr->c_other_clr.accent_clr);
            } else {
                line_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
                    var->c_watermark.watermark_color[0],
                    var->c_watermark.watermark_color[1],
                    var->c_watermark.watermark_color[2],
                    var->c_watermark.watermark_color[3]));
            }
            ...
            for (int i = 0; i < function.size(); i++)
            {
                ...
                gui->sameline();
                // Use accent/RGB color for separator
                draw->add_rect_filled(draw_list, ImGui::GetCursorScreenPos() - SCALE(12, 0), ImGui::GetCursorScreenPos() + SCALE(-9, 14), line_col, 10.f);
                gui->sameline();
            }
...
std::string c_gui::get_current_date()
{
    ...
    char date_buf[16];
    snprintf(date_buf, sizeof(date_buf), "%02d/%02d/%04d", st.wDay, st.wMonth, st.wYear);

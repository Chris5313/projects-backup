...
// Per-color rainbow state — keyed by color array pointer address.
// MUST NOT use std::unordered_map — CRT is not initialized in this
// manually PE-mapped DLL, and the map's internal lock init crashes
// inside ntdll!RtlInitializeResource.
struct ColorRainbow { uintptr_t key = 0; bool enabled = false; float speed = 1.0f; };
static ColorRainbow g_rainbow_slots[64] = {};

static ColorRainbow& GetOrCreateSlot(uintptr_t col_ptr) {
    for (auto& s : g_rainbow_slots)
        if (s.key == col_ptr) return s;
    for (auto& s : g_rainbow_slots)
        if (!s.key) { s.key = col_ptr; return s; }
    return g_rainbow_slots[0]; // full — overwrite first slot (shouldn't happen with <64 colors)
}

void UpdatePerColorRainbow() {
    float time = ImGui::GetTime();
    for (auto& rb : g_rainbow_slots) {
        if (!rb.key || !rb.enabled) continue;
        float* col = (float*)rb.key;
        float hue = fmodf(time * rb.speed * 0.2f, 1.0f);
        float r, g, b;
...
bool IsColorRainbow(float* col) {
    uintptr_t p = (uintptr_t)col;
    for (auto& s : g_rainbow_slots)
        if (s.key == p) return s.enabled;
    return false;
}
...
bool color_button(const char* desc_id, const ImVec4& col, bool opened)
{
    ...
    char id_buf[128]; snprintf(id_buf, sizeof(id_buf), "%scolor_button", desc_id);
    ...
    const ImRect row_rect(pos, pos + ImVec2(w, h));
    const ImRect swatch_rect(pos + ImVec2(w - SCALE(30), SCALE(5)), pos + ImVec2(w - SCALE(10), h - SCALE(5)));

    ItemSize(row_rect, 0.f);
    if (!ItemAdd(row_rect, id))
        return false;

    bool hovered, held;
    bool pressed = ButtonBehavior(row_rect, id, &hovered, &held, ImGuiButtonFlags_MouseButtonLeft | ImGuiButtonFlags_MouseButtonRight);

    // Label text — same convention as checkbox rows (left aligned, vertically centered).
    // A leading "##" hides the label (e.g. the swatch inside checkbox color popups).
    const char* label_end = FindRenderedTextEnd(desc_id);
    const ImVec4 text_colored = opened ? clr->c_text.text_active : hovered ? clr->c_text.text_hov : clr->c_text.text;
    draw->render_text(window->DrawList, set->c_font.inter_medium[0], row_rect.Min, row_rect.Max, gui->get_clr(text_colored), desc_id, label_end, NULL, { 0.0, 0.5 });

    // Real color swatch: checkerboard shows through alpha, color fill, subtle border.
    const ImU32 col32 = ImGui::ColorConvertFloat4ToU32(col);
    render_color_rect_with_alpha_checkboard(window->DrawList, swatch_rect.Min, swatch_rect.Max, col32, SCALE(5.f), ImVec2(0, 0), SCALE(set->c_element.rounding), 0);
    draw->add_rect(window->DrawList, swatch_rect.Min, swatch_rect.Max, gui->get_clr(opened ? clr->c_other_clr.accent_clr : clr->c_child.stroke), SCALE(set->c_element.rounding), 0, SCALE(1.f));
...
bool color_picker(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    char anim_buf[64]; snprintf(anim_buf, sizeof(anim_buf), "%usaved_pos", GetCurrentWindow()->ID);
    picker_state* animstate = gui->anim_container(&animstate, GetID(anim_buf));
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        ImGuiColorEditFlags sub_flags = (flags & sub_flags_to_forward) | ImGuiColorEditFlags_NoPicker | ImGuiColorEditFlags_NoSmallPreview;
...
bool c_widget::color_edit(std::string_view label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (widget->text_field("##Text", "\xef\x8a\x92", buf, IM_ARRAYSIZE(buf), SCALE(150, 30), ImGuiInputTextFlags_CharsUppercase))
        {
            value_changed = true;
            char* p = buf;
            ...
            auto hexval = [](char c) -> int { return (c >= '0' && c <= '9') ? c - '0' : (c >= 'a' && c <= 'f') ? c - 'a' + 10 : (c >= 'A' && c <= 'F') ? c - 'A' + 10 : 0; };
            auto hex2 = [&](const char* s) -> unsigned { return (hexval(s[0]) << 4) | hexval(s[1]); };
            i[0] = hex2(p);
            i[1] = hex2(p + 2);
            i[2] = hex2(p + 4);
            if (alpha)
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        const bool row_pressed = color_button(label.data(), col_v4, state->active);
        const ImRect row_rect = g.LastItemData.Rect;

        // Click (LMB or RMB) on the row toggles the picker popup;
        // clicking anywhere outside the row and the popup closes it.
        if (row_pressed)
        {
            state->active = !state->active;
            g.ColorPickerRef = col_v4;
        }
        else if (state->active && (g.IO.MouseClicked[0] || g.IO.MouseClicked[1]) && !state->hovered && !row_rect.Contains(g.IO.MousePos))
            state->active = false;
        if (!state->active)
            state->hovered = false;
        ...
        SetNextWindowPos(ImVec2(row_rect.Max.x - SCALE(30), row_rect.Max.y));
        ...
        {
            char picker_buf[64]; snprintf(picker_buf, sizeof(picker_buf), "picker_window%u", GetID(label.data()));
            gui->begin(picker_buf, NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_AlwaysAutoResize);
            {
                if (state->active)
                    SetWindowFocus();
                ...
                // Per-color rainbow toggle
                widget->separator();
                auto& rb = GetOrCreateSlot((uintptr_t)col);
                widget->checkbox("Rainbow", &rb.enabled);
                if (rb.enabled)
                    widget->slider_float("Speed", &rb.speed, 0.1f, 5.0f, 0.1f, "%.1fx");

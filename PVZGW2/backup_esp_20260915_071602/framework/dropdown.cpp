...
bool selectable_ex(std::string_view label, bool active, ImDrawFlags rounded_flag)
{
    ...
    if (pressed) { MarkItemEdited(id); ui_sound::click(); }
...
bool dropdown_list(std::string_view label, std::string_view preview_value, int val, int max_count, ImGuiComboFlags flags, bool multi)
{
    ...
    draw->render_text(set->c_font.icon[0], window->DrawList, open_combo.Max - ImVec2(rect_size, rect_size), open_combo.Max, gui->get_clr(clr->c_text.text), "\xef\x81\xb8", NULL, NULL, { 0.5, 0.60 }, NULL);
...
bool tool_dropdown_list(std::string_view label, std::string_view preview_value, int val, int max_count, ImGuiComboFlags flags, bool multi)
{
    ...
    draw->render_text(set->c_font.icon[0], window->DrawList, rect.Max - ImVec2(SCALE(36.f), SCALE(36.f)), rect.Max, gui->get_clr(clr->c_text.text), "\xef\x81\xb8", NULL, NULL, { 0.5, 0.50 }, NULL);

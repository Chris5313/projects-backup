...
static void DrawEsp()
{
    ...
    // v25 DEBUG: log first 3 entity positions and distances
    static DWORD s_posLog = 0;
    if (n > 0 && GetTickCount() - s_posLog >= 4000) {
        s_posLog = GetTickCount();
        char pb[256]; int pk = 0;
        const char* hdr = "ENT_POS: n=";
        while (*hdr) pb[pk++] = *hdr++;
        // inline int
        if (n < 10) pb[pk++] = '0' + n;
        else { pb[pk++] = '0' + n/10; pb[pk++] = '0' + n%10; }
        hdr = " max_dist=";
        while (*hdr) pb[pk++] = *hdr++;
        int md = var->c_esp.max_dist;
        if (md < 10) pb[pk++] = '0' + md;
        else if (md < 100) { pb[pk++] = '0' + md/10; pb[pk++] = '0' + md%10; }
        else { pb[pk++] = '0' + md/100; pb[pk++] = '0' + (md/10)%10; pb[pk++] = '0' + md%10; }
        for (int i = 0; i < 3 && i < n && pk < 200; i++) {
            hdr = " [";
            while (*hdr) pb[pk++] = *hdr++;
            pb[pk++] = '0' + i;
            hdr = "] d=";
            ...
            int di = (int)ds[i];
            if (di < 10) pb[pk++] = '0' + di;
            else if (di < 100) { pb[pk++] = '0' + di/10; pb[pk++] = '0' + di%10; }
            else if (di < 1000) { pb[pk++] = '0' + di/100; pb[pk++] = '0' + (di/10)%10; pb[pk++] = '0' + di%10; }
            else { hdr = ">999"; while (*hdr) pb[pk++] = *hdr++; }
        }
        pb[pk] = 0;
        OILog(pb);
    }

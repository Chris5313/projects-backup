...
static HRESULT __stdcall PresentHook(IDXGISwapChain* sc, UINT sync, UINT flags)
{
    ...
    // Release: F1 (menu) is the ONLY bind — the entity refresh is automatic
    // and the triggerbot toggles from the Combat tab. Debug builds keep
    // F2 (full dump scan) / F3 (dump pair) / F5 (local dump) / F6 (trig).
#ifndef GW2_RELEASE
    ...
    // F3 — automated HP dump pair (debug only).
#ifndef GW2_RELEASE
    // dump 1, 15s countdown (take hits!), dump 2, DONE banner. Marker file
    // C:\Users\Public\gw2_hp_pair_done.txt lands with the second dump.
    {
        ...
        if (k & 0x8000) {
            if (!f3Down) {
                f3Down = true;
                if (g_autoBusy) {
                    Log("F3 — ignored (auto pair already running)");
                } else {
                    InterlockedExchange(&g_autoBusy, 1);
                    Log("F3 — auto pair launched");
                    CreateThread(nullptr, 0, AutoDumpThread, nullptr, 0, nullptr);
                }
            }
        } else {
            f3Down = false;

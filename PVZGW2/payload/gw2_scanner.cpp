...
// ---- pass E: find entity list/manager ----------------------------------------
// After entity discovery (pass A+B), scan heap for pointers TO those entities.
// Consecutive entity pointers indicate an array; parent struct = entity manager.
static uint64_t g_entAddrs[256];
static int      g_nEntAddrs = 0;

static int EntFind(uint64_t v)
{
    int lo = 0, hi = g_nEntAddrs - 1;
    while (lo <= hi) {
        int m = (lo + hi) >> 1;
        if (g_entAddrs[m] == v) return m;
        if (g_entAddrs[m] < v) lo = m + 1; else hi = m - 1;
    }
    return -1;
}

// Track consecutive entity pointers (candidate arrays)
struct PtrRun { uint64_t base; int count; };
static PtrRun g_runs[512];
static int    g_nRuns = 0;
static uint64_t g_lastHitAddr = 0;
static int      g_runLen = 0;
static uint64_t g_runBase = 0;

static void FlushRun()
{
    if (g_runLen >= 2 && g_nRuns < 512) {
        g_runs[g_nRuns].base = g_runBase;
        g_runs[g_nRuns].count = g_runLen;
        g_nRuns++;
    }
    g_runLen = 0;
}

static void ScanE(uint64_t base, size_t len)
{
    if (g_nEntAddrs == 0) return;
    size_t nq = len >> 3;
    const uint64_t* qv = (const uint64_t*)g_snap;
    for (size_t i = 0; i < nq; i++) {
        uint64_t v = qv[i];
        if (EntFind(v) >= 0) {
            uint64_t addr = base + (uint64_t)(i << 3);
            // Log individual hit
            char b[100]; size_t k = 0;
            const char* t = "ENTPTR ";
            while (*t) b[k++] = *t++;
            k += Hex64(b + k, addr);
            b[k++] = ' '; b[k++] = '-'; b[k++] = '>'; b[k++] = ' ';
            k += Hex64(b + k, v);
            L(b, k);
            // Track runs (consecutive qwords pointing to entities)
            if (g_runLen > 0 && addr == g_lastHitAddr + 8) {
                g_runLen++;
            } else {
                FlushRun();
                g_runBase = addr;
...
static void ScanECb(uint64_t base, size_t len) { ScanE(base, len); }
...
void RunScan()
{
    ...
    // entity layer: resolve vtable VAs (incremental - never reset ESP list)
    ent::BeginCollect(IMG_LO, false);
    ...
    // ---- pass E: find pointers TO entity objects ----
    // Get entity addresses discovered in pass A+B, sort for binary search
    g_nEntAddrs = ent::GetEntityAddrs(g_entAddrs, 256);
    if (g_nEntAddrs > 1) {
        // insertion sort
        for (int i = 1; i < g_nEntAddrs; i++) {
            uint64_t v = g_entAddrs[i];
            int j = i - 1;
            while (j >= 0 && g_entAddrs[j] > v) { g_entAddrs[j + 1] = g_entAddrs[j]; j--; }
            g_entAddrs[j + 1] = v;
        }
    }
    {
        char hdr[64]; size_t k = 0;
        const char* t = "==== PASS E entities=";
        while (*t) hdr[k++] = *t++;
        char nb[12]; int nl = 0; int x = g_nEntAddrs;
        if (!x) nb[nl++] = '0';
        while (x && nl < 12) { nb[nl++] = '0' + (x % 10); x /= 10; }
        while (nl) hdr[k++] = nb[--nl];
        hdr[k++] = ' '; hdr[k++] = '='; hdr[k++] = '='; hdr[k++] = '='; hdr[k++] = '=';
        L(hdr, k);
    }
    // Reset run tracker
    g_nRuns = 0; g_runLen = 0; g_lastHitAddr = 0; g_runBase = 0;
    // Scan heap for pointers to entities
    p = (uint64_t)si.lpMinimumApplicationAddress;
    while (p < SCAN_HI) {
        MEMORY_BASIC_INFORMATION mbi;
        if (!VirtualQuery((void*)p, &mbi, sizeof mbi)) { p += PAGE; continue; }
        uint64_t end = (uint64_t)mbi.BaseAddress + mbi.RegionSize;
        if (!GoodRegion(mbi)) { p = end; continue; }
        WalkRegion(p, end, PAGE, ScanECb);
        p = end;
    }
    FlushRun(); // flush final run
    // Log entity pointer runs (potential entity arrays)
    Ls("==== ENTITY POINTER RUNS (consecutive) ====");
    for (int i = 0; i < g_nRuns; i++) {
        char b[96]; size_t k = 0;
        const char* t = "ENTRUN ";
        while (*t) b[k++] = *t++;
        k += Hex64(b + k, g_runs[i].base);
        b[k++] = ' '; b[k++] = 'c'; b[k++] = 'o'; b[k++] = 'u'; b[k++] = 'n'; b[k++] = 't'; b[k++] = '=';
        char nb[12]; int nl = 0; int c = g_runs[i].count;
        if (!c) nb[nl++] = '0';
        while (c && nl < 12) { nb[nl++] = '0' + (c % 10); c /= 10; }
        while (nl) b[k++] = nb[--nl];
        L(b, k);
    }
    Ls("==== PASS E DONE ====");

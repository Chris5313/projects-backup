...
bool c_widget::keybind(std::string_view label, int* key)
{
    ...
    // Bounds-checked key name — a stale value past the table would read garbage.
    static const int KEY_COUNT = (int)(sizeof(keys) / sizeof(keys[0]));
    const char* key_name = (*key > 0 && *key < KEY_COUNT) ? keys[*key] : nullptr;

    if (key_name && g.ActiveId != id) {
        strcpy_s(buf_display, key_name);
    }
    ...
    draw->render_text(set->c_font.icon[1], window->DrawList, decorate_bb.Min, decorate_bb.Max, gui->get_clr(clr->c_other_clr.accent_clr), "\xef\x84\x9c", NULL, NULL, { 0.5, 0.5 }, NULL);

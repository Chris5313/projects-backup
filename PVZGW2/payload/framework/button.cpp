...
bool c_widget::tool_button(std::string_view label, std::string_view icon, const ImVec2& size) {
    ...
    if (icon != "")
        draw->render_text(window->DrawList, set->c_font.icon[4], rect.Min, rect.Max, gui->get_clr(clr->c_text.text), "\xef\x88\x85", NULL, NULL, ImVec2(0.18f, 0.5f));
    ...
    if (icon != "")
        draw->render_text(window->DrawList, set->c_font.icon[4], rect.Min, rect.Max, gui->get_clr(clr->c_text.text_active, 1.f - state->alpha), "\xef\x88\x85", NULL, NULL, ImVec2(0.18f, 0.5f));

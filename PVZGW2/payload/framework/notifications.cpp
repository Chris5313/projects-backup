...
{
    notifications.push_back({ notify_count++, std::string(text), notify_delay, 0.f, 0.f, 0.f, true, type });
}
...
void c_notify::setup_notify()
{
    ...
    // Erase fully-faded notifications. Without this the vector grows forever and
    // the render index goes negative (OOB write into notification_y_offset[]).
    notifications.erase(
        std::remove_if(notifications.begin(), notifications.end(),
            [](const notify_state& n) { return !n.active_notify && n.notify_alpha <= 0.f; }),
        notifications.end());
}
...
void c_notify::render_notify(int notification_index, float notification_alpha, float notification_offset, float notification_duration, float notification_delay, std::string_view notification_text, notify_position notification_position)
{
    ...
    char notify_buf[32]; snprintf(notify_buf, sizeof(notify_buf), "notify%d", notification_index);
    gui->begin(notify_buf, nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove);
    {

...

        {
            ...
            gui->begin_content("content", GetContentRegionAvail() - SCALE(15, 15), { 15, 15 }, { 15, 15 });
            {
                ...
                if (var->c_selection.selection_active == 0)
                {
                    gui->begin_group();
                    {
                        gui->begin_child("aimbot", ImVec2(SCALE(450), SCALE(400)));
                        {
                            widget->section("Aimbot", "\xef\x81\x9b");
                            widget->checkbox("Enable", &var->c_aimbot.enable);
                            if (var->c_aimbot.enable) {
                                widget->separator();
                                widget->slider_int("FOV", &var->c_aimbot.fov, 10, 400, 1, "%dpx");
                                widget->separator();
                                widget->checkbox("Draw FOV", &var->c_aimbot.draw_fov);
                                widget->separator();
                                widget->slider_int("Smoothing", &var->c_aimbot.smoothing, 1, 20, 1, "%d");
                                widget->separator();
                                widget->checkbox("Hold key", &var->c_aimbot.hold_key);
                            }
                            
                            widget->separator();
                            widget->section("Silent Aim", "\xef\x81\x9b");
                            widget->checkbox("Enable##silent", &var->c_aimbot.silent_enable);
                            if (var->c_aimbot.silent_enable) {
                                widget->separator();
                                widget->slider_int("Silent FOV", &var->c_aimbot.silent_fov, 10, 500, 1, "%dpx");
                                widget->separator();
                                widget->checkbox("Draw Silent FOV", &var->c_aimbot.silent_draw_fov);
                            }
                            
                            widget->separator();
                            widget->section("Target Settings", "\xef\x81\x9b");
                            widget->dropdown("Priority", &var->c_aimbot.target_priority, var->c_aimbot.priority_list, (int)var->c_aimbot.priority_list.size());
                            widget->separator();
                            widget->dropdown("Bone", &var->c_aimbot.limb, var->c_aimbot.limb_list, (int)var->c_aimbot.limb_list.size());
                            widget->separator();
                            widget->checkbox("Enemies only", &var->c_aimbot.enemies_only);
                            widget->separator();
                            widget->checkbox("Require alive", &var->c_aimbot.require_fresh);
                            widget->separator();
                            widget->slider_int("Max distance", &var->c_aimbot.max_dist, 10, 200, 1, "%dm");
                        }
                        gui->end_child();

                        gui->begin_child("triggerbot", ImVec2(SCALE(450), SCALE(120)));
                        {
                ...
                if (var->c_selection.selection_active == 1)
                {
                    gui->begin_group();
                    {
                        gui->begin_child("esp", ImVec2(SCALE(450), SCALE(320)));
                        {
                            ...
                            if (var->c_esp.enable) {
                                ...
                                widget->separator();
                                widget->checkbox("Fill box", &var->c_esp.fill_box);
                                widget->separator();
                                widget->dropdown("Filter", &var->c_esp.filter, var->c_esp.filter_list, (int)var->c_esp.filter_list.size());
                                widget->separator();
                                widget->checkbox("Distance", &var->c_esp.distance);
                                widget->separator();
                                widget->checkbox("Show NPCs", &var->c_esp.show_ai);
                                if (var->c_esp.show_ai) {
                                    widget->separator();
                                    widget->slider_float("NPC opacity", &var->c_esp.npc_alpha, 0.05f, 1.0f, 0.05f, "%.2f");
                                }
                                widget->checkbox("Status info", &var->c_esp.status_info);
                                widget->separator();
                                widget->slider_int("Max distance", &var->c_esp.max_dist, 10, 400, 1, "%dm");
                            }
                        }
                        gui->end_child();

                        gui->begin_child("esp colors", ImVec2(SCALE(450), SCALE(320)));
                        {
                            widget->section("ESP Colors", "\xef\x81\xae");
                            if (var->c_esp.enable) {
                                widget->color_edit("Plants", var->c_esp.plant_color);
                                widget->separator();
                                widget->color_edit("Zombies", var->c_esp.zombie_color);
                                widget->separator();
                                widget->color_edit("Unknown team", var->c_esp.unknown_color);
                            }
                        }
                        gui->end_child();

                        gui->begin_child("chams", ImVec2(SCALE(450), SCALE(220)));
                        {
                ...
                if (var->c_selection.selection_active == 2)
                {
                    gui->begin_group();
                    {
                        gui->begin_child("weapon mods", ImVec2(SCALE(450), SCALE(180)));
                        {
                        ...
                        gui->begin_child("movement", ImVec2(SCALE(450), SCALE(200)));
                        {
                ...
                if (var->c_selection.selection_active == 3)
                {
                    gui->begin_group();
                    {
                        gui->begin_child("config placeholder", ImVec2(SCALE(450), SCALE(120)));
                        {
                ...
                if (var->c_selection.selection_active == 4)
                {
                    gui->begin_group();
                    {
                        gui->begin_child("appearance", ImVec2(SCALE(450), SCALE(280)));
                        {
                        ...
                        gui->begin_child("misc settings", ImVec2(SCALE(450), SCALE(120)));
                        {
...
// Watermark-only render - called every frame (even when menu closed)
void GW2_Framework_RenderWatermark()
{
    if (!s_framework_inited) return;
    
    // Watermark (always visible when enabled, draggable when menu open)
    gui->water_mark("watermark", var->c_watermark.watermark_content,
        static_cast<watermark_position>(0),  // position param unused now

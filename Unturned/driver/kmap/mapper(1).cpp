...
NTSTATUS MapUserDll(PEPROCESS target, PVOID dllBuf)
{
    ...
    HANDLE hSection = nullptr;
    {
        ...
        if (!NT_SUCCESS(s2)) {
            KLogHex("ZwCreateSection failed: ", s2);
            return s2;
        }
    }
    ...
    do {
        ...
        for (USHORT i = 0; i < numSections; i++) {
            ...
            if (!NT_SUCCESS(protStatus)) {
                KLogHex("VProtect FAIL sec: ", i);
                KLogHex("  status: ", protStatus);
            }
        }
    ...
    if (!NT_SUCCESS(s) || !base) {
        KLogHex("MapUserDll: skipping APC dispatch, status=", (ULONG_PTR)s);
        return s;
    }
    ...
    if (entryRva) {
        ...
        KLogHex("entry point: ", (ULONG_PTR)ep);
        s = HijackAndInject(target, ep, base);
    } else {
        KLog("no entry point, skipping hijack");
        s = STATUS_SUCCESS;
    }
    ...
    if (NT_SUCCESS(s) && base) {
        ...
        KLog("FAIL: DllMain never executed after 15s");
    }

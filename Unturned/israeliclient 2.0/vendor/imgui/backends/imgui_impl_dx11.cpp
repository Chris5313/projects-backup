...
#include <stdio.h>
#include <d3d11.h>
//#include <d3dcompiler.h>  // NOT statically linked — use precompiled shaders
//#pragma comment(lib, "d3dcompiler")
#include "imgui_shaders.h"  // precompiled VS + PS blobs
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    {
        // Use precompiled vertex shader (no D3DCompiler dependency)
        if (bd->pd3dDevice->CreateVertexShader(g_vs_main, sizeof(g_vs_main), nullptr, &bd->pVertexShader) != S_OK)
            return false;
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, g_vs_main, sizeof(g_vs_main), &bd->pInputLayout) != S_OK)
            return false;
    ...
    {
        // Use precompiled pixel shader (no D3DCompiler dependency)
        if (bd->pd3dDevice->CreatePixelShader(g_ps_main, sizeof(g_ps_main), nullptr, &bd->pPixelShader) != S_OK)
            return false;
    }

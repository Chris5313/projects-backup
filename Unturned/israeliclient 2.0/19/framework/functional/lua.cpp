...
bool c_widget::lua_tool_button(std::string_view icon, std::string_view aid)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    char id_buf[64]; snprintf(id_buf, sizeof(id_buf), "%s%s", icon.data(), aid.data());
...
bool c_widget::lua_selectable(lua_data* data, int lua_id)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    char id_buf[64]; snprintf(id_buf, sizeof(id_buf), "%s%d", data->name.data(), lua_id);
    const ImGuiID id = window->GetID(id_buf);
    ...
    char load_buf[80]; snprintf(load_buf, sizeof(load_buf), "%s%u", data->name.data(), id);
    load_button(load_buf, &data->active);

    SetCursorScreenPos(ImVec2(rect.Max.x - SCALE(146), rect.Min.y + SCALE(23)));
    char id_str1[16]; snprintf(id_str1, sizeof(id_str1), "%u", id);
    bool pressed = lua_tool_button("B", id_str1);

    SetCursorScreenPos(ImVec2(rect.Max.x - SCALE(180), rect.Min.y + SCALE(23)));
    char id_str2[16]; snprintf(id_str2, sizeof(id_str2), "%u", id);
    if (lua_tool_button("C", id_str2))

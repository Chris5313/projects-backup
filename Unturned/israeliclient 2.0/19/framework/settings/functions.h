...
inline c_text_editor* text_editor = nullptr;

// ============================================================
// Sound system — embedded WAV data, no files on disk
// ============================================================
#ifdef BUILD_DLL
#include <mmsystem.h>
...
namespace ui_sound {
    inline void play(const unsigned char* data, size_t) {
        PlaySoundA((LPCSTR)data, NULL, SND_MEMORY | SND_ASYNC | SND_NODEFAULT);
    }
    inline void click()      { play(click_wav, sizeof(click_wav)); }
    inline void hover()      { play(hover_wav, sizeof(hover_wav)); }
    inline void toggle_on()  { play(toggle_on_wav, sizeof(toggle_on_wav)); }
    inline void toggle_off() { play(toggle_off_wav, sizeof(toggle_off_wav)); }
    inline void tab_switch() { play(tab_switch_wav, sizeof(tab_switch_wav)); }
    inline void error()      { play(error_wav, sizeof(error_wav)); }
    inline void notify()     { play(notification_wav, sizeof(notification_wav)); }
}
#else
namespace ui_sound {
    inline void click() {}
